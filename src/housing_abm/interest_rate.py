...
def update_mortgage_rate(model):
    ...
    model.mortgage_rate_spread_annual = min(
        max(model.mortgage_rate_spread_annual, cfg["spread_floor"]), cfg["spread_ceiling"]
    )
    model.mortgage_rate_annual = model.current_fed_rate_annual + model.mortgage_rate_spread_annual
    model.mortgage_rate_monthly = model.mortgage_rate_annual / 12.0

    model.mortgage_rate_history.append(model.mortgage_rate_annual)
    model.mortgage_rate_avg = sum(model.mortgage_rate_history[-12:]) / len(model.mortgage_rate_history[-12:])
