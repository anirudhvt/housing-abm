...
from housing_abm.equations.market_matching import sample_bid_up_multiplier, max_rounds, expected_gross_rental_yield
...
def _preferred_offer(bid, affordable_offers, model):
    ...
    if _is_investor(bid["agent"]):
        def yield_key(unit):
            ...
            rent_estimate = unit.rent if unit.rent is not None else tract.rent_per_quality * unit.quality
            return expected_gross_rental_yield(rent_estimate, unit.price, tract.avg_days_on_market())
        return max(affordable_offers, key=yield_key) #search through affordable households for best yield
    return max(affordable_offers, key=lambda unit: unit.quality) #everyone else looks for quality
...
def _settle_purchase(model, unit, agent, down_payment, final_price):
    ...
    if previous_owner is not None:
        ...
        if getattr(previous_owner, "properties", None) is not None and unit in previous_owner.properties:
    ...
    principal = max(unit.price - down_payment, 0.0) #never negative, regardless of  down_payment sizing
    ...
    model._monthly_new_lending += principal #feeds EQ16's spread update at end of month
...
def run_ownership_market(model):
    ...
    for unit in for_sale:
        ...
        is_investor_listing = getattr(model._resale_sellers.get(unit), "properties", None) is not None
        multiplier = price_cfg["vacancy_tax_multiplier_investor"] if is_investor_listing else 1.0
    ...
    n_rounds = max_rounds(
        n_bids = len(remaining_bids), n_offers = len(remaining_offers),
    ...
    for _round in range(n_rounds):
        ...
        for bid in remaining_bids:
            affordable = [u for u in remaining_offers
