...
class AtlantaHousingModel(Model):
    def __init__(self, config_path: str = "config/baseline_params.yaml",
        ...
        self._resale_sellers = {} #HouseholdUnit -> agent, keeps tracks of resale listings
        ...
        for _ in range(n_households):
            income = float(self.random_gen.lognormal(mean = 8.0, sigma = 0.55)) #realistic terms, generated by Claude - average ~$2,980 per month
    ...
    def _promote_to_repeat_buyer(self, ftb): #promote when FTB has been in their house for reqiured amount of time
        """carries over financial state, repoint reference to new agent"""
        rb = RepeatBuyer(model = self, income= ftb.income, age=ftb.age, tract_id = ftb.tract_id)
    ...
    def houses_per_capita(self, tract_id):
        ...
        return len(self.for_sale_units) / max(1, len([agent for agent in self.agents if hasattr(agent, "income")])) #houses/non house agents
    ...
    def houses_per_capita_avg(self, tract_id):
        ...
        if not history:
            return self.houses_per_capita(tract_id) #if no history, return current value
