...
def process_deaths(model):
    ...
    deceased = [a for a in households if model.random_gen.random() < monthly_death_probability(a.age, cfg)] #
...
def _vacate_and_delist(model, agent):
    ...
    model._ownership_bid_queue[:] = [b for b in model._ownership_bid_queue if b["agent"] is not agent]
