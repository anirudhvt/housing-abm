...
class InstitutionalInvestor(HouseholdAgent):
    ...
    def step(self):
        net_rental_cash_flow = self._collect_rent_and_pay_mortgages() #analogous to landlords, collect rent and pay mortgage
    ...
    def _collect_rent_and_pay_mortgages(self) -> float:
        total_rent = sum(u.rent or 0.0 for u in self.properties if u.tenant is not None) #only collect rent if someone lives there
        total_mortgage = sum(u.mortgage_payment or 0.0 for u in self.properties) #pay all mortgages
    ...
    def _reprice_vacant_units(self):
        ...
        for unit in self.properties:
            if unit.on_rental_market and unit.tenant is None: #currently selling
                ...
                unit.rent = institutional_rent(market_rate, premium=premium) #reprice at market rate + premium
    ...
    def _evaluate_sell_decision(self):
        ...
        for unit in list(self.properties): #see if they should sell any unit
            ...
            g = price_appreciation_expectation(
                tract.hpi_history, alpha=self.model.params["appreciation_eq4"]["alpha_institutional"]
            )
    ...
    def _evaluate_buy_decision(self):
        ...
        down_payment, is_cash = down_payment_investor(
            price=target_price, wealth=self.available_capital, agent_type="institutional",
            mu=down_cfg["mu"], sigma=down_cfg["sigma"], p_cash=down_cfg["p_cash"],
